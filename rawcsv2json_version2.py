...
for file in os.listdir("D:/Documents/UCU/Courses/Data Analysis/Assignment 6 - Wikipedia/years/"):
    ...
    print (file)
    for entry in out:
        if "22-rdf-syntax-ns#type_label" in entry:
        
            if entry["22-rdf-syntax-ns#type_label"] == 'Actor' or entry["22-rdf-syntax-ns#type_label"] == 'Actress':
                actors.append(entry)
        
            else: 
                split_occupation = entry["22-rdf-syntax-ns#type_label"][1:-1].split('|')
                for i in range(len(split_occupation)):
                    if split_occupation[i] == 'Actor' or split_occupation[i] == 'Actress' or split_occupation[i] == 'Actor109765278':
                        actors.append(entry)
                            
with open('actors_big.json', 'w') as file:                        
